...
import { kubernetesServicesTableKpis, ServiceTableKpiKeys } from 'constants';
...
const serviceTableKpiByKey = kubernetesServicesTableKpis.reduce(
...
const useKpisByServiceNameRequest = () => {
  ...
  const fetchSingleColumn = ({ date, key, selectedFacetValuesByName }) => {
    const kpi = kubernetesServicesTableKpis.find((kpi) => kpi.key === key);
    if (kpi) {
