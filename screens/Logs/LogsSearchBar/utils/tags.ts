...
export const getFilterByFacetTags = (
  ...
  onEditFilterByFacet: (val: string, index: number) => void,
): Array<SearchBarTagsProps> => {
  return facetQueries.map((facetQuery, index) => ({
    ...
    onEdit: () => {
      onEditFilterByFacet(facetQuery, index);
    },
  }));
};
